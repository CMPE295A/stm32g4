...
{
	bool detected = false;
    I2C1->CR2 = (slave_address << 1) | I2C_CR2_START; // Send start condition
    while (!(I2C1->ISR & I2C_ISR_BUSY)); // Wait for start condition to be sent
    while (!(I2C1->ISR & I2C_ISR_ADDR)); // Wait for end of address transmission
    (void)I2C1->SR1; // Clear ADDR flag by reading SR1 and SR2 registers
    (void)I2C1->SR2;
    if ((I2C1->ISR & I2C_ISR_NACKF) || (I2C1->ISR & I2C_ISR_ARLO)) { // ACK received
    	detected = true;
    }
    return detected;
}
